...
## 67. 5.40409错误：查询记录出错，[用户‘sa’登录失败。（-2147203048）select count(*) from ESSystem.ES_Seslog where[sesId]=?] 

> @XY：
原因：server.ini 中记录着sa密码的加密结果，升级的时候要读它解密然后连数据库，因为server.ini 以及其它的一起解密用的dll和exe不在同一目录下（和Excel.exe 也不在同一目录下），程序就去环境变量path 中找相应的库，加解密用的开源库，别的软件可能的也用到，那些软件用的同名的dll 安在了system32里，而它的路径排在ES的路径之前，所以ES的软件按path 的顺序找，先找到了它们的软件，就用了，但是解密解不开（所以就是sa登录失败，因为sa的口令没能得到） 

之所以系统管理台不报错，是因为系统管理台的exe就在安装目录下，ES软件用的所有文件也在安装目录下，windows操作系统就是这样，它会先在安装目录下找，找不到了再到系统环境变量path中写的那些目录下去找。而Excel的运行文件excel.exe是在Office的安装目录下，和ES软件安装目录不在一起，所以它就先用了path的目录，而path 的目录，也是按从前到后的顺序找的，找到第一个就用不再往后找。 
ES的路径在path中排在最后，所以excel.exe找到的总是别人的同名文件。 

处理方法：在path中将ES程序路径排到前面 
